    ...
    public class CFCache
{
    ...
    public static void SysCount(int minute)
    {
        SysCount(minute, 0, null);
    }

    public static void SysCount(int minute, int bz, string refstr)
    {
        if (!IsRequestedNotSet(minute))
        {
            HttpContext current = HttpContext.Current;
            if (refstr == null)
            {
                try
                {
                    refstr = current.Request.ServerVariables["HTTP_referer"];
                    if (refstr == null)
                    {
                        return;
                    }
                    refstr = refstr.Replace('\'', ' ').Trim();
                    string host = PubFunc.GetHost(refstr);
                    if (host.EndsWith(CFConfig.QQDomain) || host.EndsWith("5imybbs.com"))
                    {
                        return;
                    }
                    if (host.EndsWith(CFConfig.QQDomain))
                    {
                        refstr = "I";
                    }
                    else if (PubTypes.GetByteLength(refstr) > 0xff)
                    {
                        refstr = refstr.Substring(0, 0x7f);
                    }
                }
            ...
            else if (refstr.ToLower().IndexOf(CFConfig.QQDomain) >= 0)
            {
                refstr = "I";
            }
            else
            {
                ...
                if (PubTypes.GetByteLength(refstr) > 0xff)
                {
                    refstr = refstr.Substring(0, 0x7f);
                }
            }
            SqlCommand command = new SqlCommand("p_count_tj", CFConfig.GetConnection());
            try
            {
                command.CommandType = CommandType.StoredProcedure;
                SqlParameterCollection parameters = command.Parameters;
                parameters.Add("@ip", SqlDbType.VarChar, 15).Value = FWFunc.GetIP();
                parameters.Add("@ref", SqlDbType.VarChar, 0xff).Value = refstr;
                parameters.Add("@bz", SqlDbType.Int).Value = bz & 0xff;
                parameters = null;
                command.ExecuteNonQuery();
            }
            finally
    ...
    public static bool ZuJi(char ltype, int rid)
    {
        int cookieCID = CFCookies.GetCookieCID(false);
        if (cookieCID == -1)
        {
            return false;
        }
        if (!IsRequested(60))
        {
            SqlCommand command = new SqlCommand("p_zuji", CFConfig.GetConnection());
            try
            {
                command.CommandType = CommandType.StoredProcedure;
                SqlParameterCollection parameters = command.Parameters;
                parameters.Add("@ltype", SqlDbType.Char, 1).Value = ltype;
                parameters.Add("@cid", SqlDbType.Int).Value = cookieCID;
                parameters.Add("@rid", SqlDbType.Int).Value = rid;
                parameters = null;
                command.ExecuteNonQuery();
            }
            finally
